...
2. **Revisa el historial reciente**: Consulta [CHANGELOG.md](file:///c:/Users/Martin/.gemini/antigravity-ide/scratch/gastrofest-app/CHANGELOG.md) para conocer la última versión desplegada (`v1.8.0`) y las tareas en curso en la sección `[Unreleased]`.
3. **Valida el estado de la UI y los Tests**: Ejecuta `node test_all_app_buttons.js` y `node test_multiagent_simulation.js` para asegurar que las 140 pruebas sigan pasando al 100%.

---

## 🏗️ 2. Resumen Técnico Rápido
* **Propósito**: PWA para festival gastronómico con modal de bienvenida multicanal (Google / Facebook / Invitado), base de datos de leads para marketing con exportación CSV, radar en vivo, agenda de espectáculos, stands con filtros dietarios (Vegano/Sin TACC), sorteo con Golden Ticket QR interactivo, mapa satelital interactivo con zoom por pellizco (Pinch-to-zoom) y panel de administración para operadores con acceso por PIN.
...
* **Test de Botones, Leads, Facebook Login, Zoom & UI (95 pruebas JSDOM):** `node test_all_app_buttons.js`
