...
2. **Revisa el historial reciente**: Consulta [CHANGELOG.md](file:///c:/Users/Martin/.gemini/antigravity-ide/scratch/gastrofest-app/CHANGELOG.md) para conocer la última versión desplegada (`v1.4.0`) y las tareas en curso en la sección `[Unreleased]`.
3. **Valida el estado de la UI y los Tests**: Ejecuta `node test_all_app_buttons.js` y `node test_multiagent_simulation.js` para asegurar que las 122 pruebas sigan pasando al 100%.

---

## 🏗️ 2. Resumen Técnico Rápido
* **Propósito**: PWA para festival gastronómico con modal de bienvenida (Google / Invitado), radar en vivo, agenda de espectáculos, stands con filtros dietarios (Vegano/Sin TACC), sorteo con Golden Ticket QR interactivo, mapa interactivo de Plaza Independencia (con Zona DJ) y panel de administración para operadores con acceso por PIN.
* **Stack**: HTML5, Vanilla CSS (Dark Eco Theme), JavaScript Moderno (`js/auth.js`), Express REST API (`server.js`), LocalStorage offline cache y Sharp para compresión WebP.
...
* **Test de Botones, Auth & UI (77 pruebas JSDOM):** `node test_all_app_buttons.js`
